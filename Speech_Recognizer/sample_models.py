...
def deep_rnn_model(input_dim, units, recur_layers, output_dim=29):
    ...
    # Main acoustic input
    input_data = Input(name='the_input', shape=(None, input_dim))
    recur_layer = input_data
    # Add recurrent layers, each with batch normalization
    for i in range (recur_layers):
        recur_layer = GRU(units, activation='relu',
        return_sequences=True, implementation=2)(recur_layer)
        recur_layer = BatchNormalization()(recur_layer)
    # Add a TimeDistributed(Dense(output_dim)) layer
    time_dense = TimeDistributed(Dense(output_dim=output_dim))(recur_layer)
    # Add softmax activation layer
    y_pred = Activation('softmax', name='softmax')(time_dense)
    # Specify the model
    model = Model(inputs=input_data, outputs=y_pred)
...
def final_model(input_dim, filters, kernel_size, conv_stride, conv_border_mode, units, output_dim=29):
    ...
    bidir_rnn = Bidirectional(GRU(units, activation='relu', return_sequences=True, implementation=2))(bn_cnn)
